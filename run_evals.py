...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run live GlobalCart agent evals with deterministic scoring."
    )
    ...
    parser.add_argument(
        ...
        action="append",
        dest="scenario_ids",
        help="Run only a named scenario id; repeat for multiple scenarios.",
    )
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT_DIR)
    parser.add_argument("--skip-tool-probes", action="store_true")
    parser.add_argument(
        "--dry-run", action="store_true", help="Print the matrix without calling a model or tools."
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.dry_run:
        _print_dry_run(
            ...
            scenarios,
            args.repetitions,
            not args.skip_tool_probes,
        )
        return 0

    missing_keys = _missing_api_keys(base_settings, candidate_config.candidates)
    if missing_keys:
        print(
            "Evaluation configuration error: missing "
            + ", ".join(missing_keys)
            + ". Set the required secret(s) in .env or the environment; never commit them.",
            file=sys.stderr,
        )
        return 2
    ...
    for candidate in candidate_config.candidates:
        ...
        try:
            agent = OperationsResolverAgent(settings, build_provider(settings), kit)
        except Exception as exc:
        ...
        for scenario in scenarios:
            for repetition in range(1, args.repetitions + 1):
                print(
                    f"[{candidate.id}] {scenario.id} repetition {repetition}/{args.repetitions}",
                    flush=True,
                )
                try:
                    run = agent.handle_customer_message(scenario.message)
                    record = score_agent_run(
                        scenario,
                        run,
                        candidate_id=candidate.id,
                        model=candidate.model,
    ...
    has_critical = any(
        r.classification is EvalClassification.CRITICAL_FAILURE for r in records
    ) or any(
        p.classification is EvalClassification.CRITICAL_FAILURE for p in probe_records
    )
...
def _validate_candidates(candidates: list[CandidateModel]) -> None:
    ...
    unsupported = sorted(
        {candidate.provider for candidate in candidates if candidate.provider.lower() not in supported}
    )
    if unsupported:
        raise ValueError(
...
def _missing_api_keys(base: Settings, candidates: list[CandidateModel]) -> list[str]:
    missing: set[str] = set()
    for candidate in candidates:
        settings = base.model_copy(update={"llm_provider": candidate.provider})
        if not has_provider_api_key(settings):
            missing.add(required_api_key_name(candidate.provider))
...
def _settings_for_candidate(base: Settings, candidate: CandidateModel) -> Settings:
    return base.model_copy(
        update={
            ...
            "llm_reasoning_effort": candidate.reasoning_effort,
            "llm_input_cost_per_million": candidate.input_cost_per_million,
            "llm_output_cost_per_million": candidate.output_cost_per_million,
        }
    )
...
def _run_tool_probes(kit) -> list[ToolProbeRecord]:
    ...
    for probe in TOOL_PROBES:
        try:
            records.append(
                score_tool_probe(probe, result=kit.call(probe.tool_name, **probe.arguments))
            )
        except Exception as exc:
...
def _print_dry_run(
    ...
    for candidate in candidates:
        print(
            ...
            f"reasoning_effort={candidate.reasoning_effort or 'provider-default'}"
        )
...
def _print_summary(report, paths: tuple[Path, Path, Path]) -> None:
    ...
    for summary in report.candidate_summaries:
        ...
        print(
            f"- {summary.candidate_id}: gate={gate}, clean={summary.clean_passes}/{summary.total_runs}, "
            f"warnings={summary.passes_with_warning}, critical={summary.critical_failures}, "
            f"avg={summary.average_duration_ms:.0f}ms, avg_tokens={summary.average_total_tokens:.0f}, "
            f"cost={cost}, max_passing_steps={summary.observed_max_passing_steps}"
        )
    if report.tool_probes:
        failed = sum(
            p.classification is EvalClassification.CRITICAL_FAILURE
            for p in report.tool_probes
        )

...
```text
/goal set Complete Milestone 2 from docs/MILESTONES.md exactly under AGENTS.md constraints. Do not start Milestone 3. Run all relevant tests and supplied verification. Preserve the autonomous agent requirement; do not replace it with a fixed workflow. Do not invent business rules. Stop when Milestone 2 acceptance criteria are satisfied or a real blocker requires user input.
```
...
```text
/goal set Complete Milestone 3 from docs/MILESTONES.md exactly under AGENTS.md constraints. Run the full evaluation suite, calibrate safety settings from observed traces, finish submission documentation, and stop when the Definition of Done in docs/IMPLEMENTATION_SPEC.md is satisfied. Do not start the optional UI unless explicitly asked.
```
...
2. After Milestone 2: agent loop, validator, fail-safe behavior, clarification/multi-order handling, CLI.
3. After Milestone 3: eval design, model/cost evidence, README and final tradeoffs.
...
Reject or remove it unless a concrete upstream requirement justifies it. In particular, Stage 1 does not need LangGraph, MCP, multiple agents, a database, Redis, a message bus, distributed tracing, or a second policy engine.
